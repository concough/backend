...
from api.v2.serializers.archive_serializers import ArchiveEntranceSetSerializer, ArchiveEntranceSerializer
from api.v2.serializers.general_serializers import EntranceTypeWithIdSerializer, EntranceSetSerializer, \
    ExaminationGroupSerializer, ExaminationGroupWithIdSerializer, EntranceTypeWithSaleSerializer
...
class EntrancesViewSet(ReadOnlyModelViewSet):
    def list(self, request, eset, **kwargs):
        queryset = Entrance.objects.annotate(booklets_count=Count('booklets'), duration=Sum('booklets__duration'))\
            ...
            .prefetch_related('organization', 'entrance_type').order_by('-year', '-month')
        serializer = ArchiveEntranceSerializer(queryset, many=True)

        entrance_type = EntranceType.objects.filter(pk=queryset[0].entrance_type.id).first()
        serializer2 = EntranceTypeWithSaleSerializer(entrance_type, many=False)
        ...
        if len(queryset) > 0:
            ...
            if entrance_type is not None:
                result["entrance_type"] = serializer2.data
            else:
                result["entrance_type"] = []
        else:
            result["status"] = "Error"
            result["error_type"] = "EmptyArray"
