...
from api.v1.serializers.activity_serializers import ActivitySerializer
from main.Helpers.model_static_values import CONCOUGH_LOG_TYPES
...
class ActivityViewSet(ReadOnlyModelViewSet):
    def list(self, request, **kwargs):
        queryset = ConcoughActivity.objects.filter(activity_type__in=CONCOUGH_LOG_TYPES).prefetch_related('target')[:10]
    ...
    def next_page(self, request, last, **kwargs):
        try:
            ...
            queryset = ConcoughActivity.objects.filter(activity_type__in=CONCOUGH_LOG_TYPES, created__lt=d).prefetch_related('target')[:10]
            serializer = ActivitySerializer(queryset, many=True)
