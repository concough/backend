...
from admin.Forms.MessageForms import MessageNewForm, MessageReplyForm
...
def message_list(request):
    ...
    if task_id == -1:
        # get all messages
        message_list_sent = TaskMessage.objects.filter(form_user=request.user)\
            .prefetch_related('message_type', 'form_user', 'to_user', 'task', 'task__entrance')\
            .order_by('seen', 'seen_time')
        message_list_received = TaskMessage.objects.filter(to_user=request.user)\
            .prefetch_related('message_type', 'form_user', 'to_user', 'task', 'task__entrance')\
            .order_by('seen', 'seen_time')
    else:
        message_list_sent = TaskMessage.objects.filter(task__id=task_id).filter(form_user=request.user)\
            .prefetch_related('message_type', 'form_user', 'to_user', 'task', 'task__entrance')\
            .order_by('seen', 'seen_time')
        message_list_received = TaskMessage.objects.filter(task__id=task_id).filter(to_user=request.user)\
            .prefetch_related('message_type', 'form_user', 'to_user', 'task', 'task__entrance')\
            .order_by('seen', 'seen_time')
        filtered = True
        task = Task.objects.prefetch_related('entrance').get(pk=task_id)
    ...
    d = dict(menul=menu_settings.menus, msel=menu_selected, messages_sent=messages_sent, messages_received=messages_received,
             ugn=user_group_name, filtered=filtered, tid=task_id, next_url=next_url, task=task)
    return render_to_response("admin/de_messages/messages.html", d,
                              context_instance=RequestContext(request))


@login_required
@group_permission_required('main.de_message_box', raise_exception=True)
def message_new(request, pk):
    menu_selected = "messages"
    has_error = False
    error_no = -1
    form = None
    ...
    try:
        task = Task.objects.prefetch_related('owner', 'entrance').get(pk=pk)
    ...
    if not has_error:
        if request.method == 'GET':
            ...
            form = MessageNewForm(request.POST, request.FILES)
            ...
            if form.is_valid():
                ...
                data = {"message": record,
                    "link": abs_url.reverse_absolute_uri("dataentry.messages")}
    ...
    d = dict(menul=menu_settings.menus, msel=menu_selected, form=form, task=task, next_url=next_url)
    d.update(csrf(request))
    return render_to_response("admin/de_messages/message_new.html", d,
                              context_instance=RequestContext(request))
...
def message_reply(request, pk):
    ...
    # find task associated
    try:
        message = TaskMessage.objects.prefetch_related('task', 'task__entrance').get(pk=pk)
    except:
        has_error = True
        error_no = 1
    ...
    if not has_error:
        if request.method == 'GET':
            ...
            if form.is_valid():
                ...
                full_redirect_url = "%s?%s" % (reverse('admin.messages'), "tid=%d" % message.task.id)
    ...
    d.update(csrf(request))
    return render_to_response("admin/de_messages/message_reply.html", d,
                              context_instance=RequestContext(request))
...
def message_seen(request, pk):
    try:
        message = TaskMessage.objects.prefetch_related('task').get(pk=pk)
        task_id = message.task.id
        message.seen = True
        message.seen_time = datetime.now()
        message.save()

        full_redirect_url = "%s?%s" % (reverse('admin.messages'), "tid=%d" % task_id)
        return redirect(full_redirect_url)
    except:
        pass
